...
function PdfViewer() {
  ...
  return (
    <Box display="flex" flexDirection="column" height="100vh" overflow="hidden">
      <Box padding="1rem">
        <Select placeholder="Select a PDF" onChange={async (e) => {
          const selectedDoc = documents.find(doc => doc.name === e.target.value);
          if (selectedDoc) {
      ...
      <Box flex="1" padding="1rem" ref={pdfRef} overflowY="auto">
        <Box display="flex" flexDirection="column" alignItems="center" minHeight="100%">
          {selectedPdfUrl && (
            <Document file={selectedPdfUrl} onLoadSuccess={({ numPages }) => setNumPages(numPages)}>
              {Array.from({ length: numPages }, (_, index) => (
                <Box key={`page_${index + 1}`} borderBottom="1px solid black" marginBottom="1rem">
